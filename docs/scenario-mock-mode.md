...
mock 応答は **台本 (`body`) の中にインライン**で書く。`<` (送信) と対称な `$> <window>: <応答>` 行として並べるので、台本とモックが 1 つのテキストで完結し、エディタからそのまま編集できる:

```
< インシデント: 九州製作所の P-2024-KYU-001 で品質トラブル。登録して。
> インシデント 90s
$> インシデント: ✅ インシデントを登録しました。\n\n| 項目 | 値 |\n|---|---|\n| incidentId | INC-... |
...
### 構文・解決規則
- `$> <window>: <応答>`: `<` と対称の構文。**window 名を明示**する (最初の `:` まで window 名、残りが応答)。応答本文に `:` が含まれても OK (非貪欲マッチで最初の `:` だけ区切る)。
...
- **表示**: mock ON 時のみ `$>` 行を表示 (mock 色)。OFF 時は `$>` 行を空に描いて消す (実行されない)。逆に ON 時は `>` 待機行を空に消す (応答源は `$>` なので)。トグルで即切替。
