...
# CH2 hosting アプリのローカル実行 (validation 用)
cd mule-app
JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64 mvn clean package -DskipTests -DattachMuleSources
mulet run target/atelier-static-1.0.5-mule-application.jar -g mule.env=local
# → http://127.0.0.1:8081/__health で起動確認 (静的ファイル配信は mulet では readUrl classpath 未対応で 404)
```
...
| 症状 | 原因 / 対策 |
|---|---|
| mulet で `/` `/styles.css` 404 | `readUrl("classpath://...")` が mulet 未対応。 CH2 では動く |
